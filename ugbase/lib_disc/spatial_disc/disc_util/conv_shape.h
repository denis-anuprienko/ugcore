...
template <typename TFVGeom>
bool
ConvectionShapesSkewedUpwind<TDim>::
update(const TFVGeom* geo,
       const MathVector<dim>* Velocity,
       const MathMatrix<dim, dim>* DiffDisp,
       bool computeDeriv)
...
//	loop subcontrol volume faces
	...
	//	should reset the interpolation adding for hanging dofs to zero
		...
		size_t up = 0;
		try{
			GetNodeNextToCut<typename TFVGeom::ref_elem_type, dim>
					(up, scvf.global_ip(), Velocity[ip], vCornerCoords);
		} UG_CATCH_THROW("GetSkewedUpwindShapes: Cannot find side intersection.");
	...
	//	Write Derivatives if wanted
		if(computeDeriv){
			for(size_t sh = 0; sh < numSH; ++sh) VecSet(D_vel(ip, sh), 0.0);
			//VecScale(D_vel(ip, up), scvf.normal(), 1.);
			D_vel(ip, up) = scvf.normal();
		}
